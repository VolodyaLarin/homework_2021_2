...
QUnit.module('Тестируем функцию zip', function () {
	...
	QUnit.test('Функция правильно работает с массивами', function (assert) {
		assert.deepEqual(zip([], { hello: 'world' }), { hello: 'world' });
		assert.deepEqual(zip(["hello"], ["world"], { age: 4 }), { 0: "hello", age: 4 });
		assert.deepEqual(zip({ name: 'age' }, ["hello"]), { 0: "hello", name: 'age' });
	});

	QUnit.test('Функция правильно работает с вложенными объектами', function (assert) {
		assert.deepEqual(zip({ one: { two: ["three"] } }), { one: { two: ["three"] } });
		assert.deepEqual(zip(
			{ one: { two: ["three"] } },
			{ one: {} },
			{ two: ["three"] },
